...
### 7. IOPS Ceilings — The "How High Can You Go?" Trap
...
> Need **>256,000 IOPS**? → Only **EC2 Instance Store** can do it.
> io2 Block Express is the highest EBS option but still has a hard 256,000 ceiling.

**io2 Block Express** = highest-performance EBS type. Built on AWS's newest storage server architecture. Sub-millisecond latency, up to 256,000 IOPS, up to 64 TiB.

### 8. gp2 IOPS Cap Trap

gp2 IOPS = 3 per GiB, but **capped at 16,000 IOPS** (reached at ~5,334 GiB).

> If your gp2 volume is already larger than ~5.3 TB, **increasing its size will NOT add more IOPS** — you've already hit the ceiling.

**Ways to increase IOPS when gp2 is maxed:**
- Switch to **io1 or io2** (Provisioned IOPS — up to 64K or 256K)
- Use **RAID 0** across multiple EBS volumes (stripes data across volumes, combining their IOPS)

**RAID 0 (Redundant Array of Independent Disks — level 0):**
- Splits data across 2+ EBS volumes simultaneously
- Total IOPS and throughput = sum of all volumes
- Example: 2× gp2 volumes = 2× the IOPS
- Trade-off: if one volume fails, all data is lost (no redundancy)
